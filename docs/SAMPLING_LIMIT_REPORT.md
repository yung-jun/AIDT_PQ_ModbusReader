...
| 方法 | 理論收益 | 限制 |
|------|---------|------|
| 提升 baud rate（38400 → 115200） | 傳輸時間縮短 3×，3 台極限降至 ~87 ms ≈ 11 Hz | 韌體延遲 60 ms 不變，仍是主瓶頸 |
| 每台電表用獨立 USB-RS485 + 多執行緒 | 3 台並行 → ~50 ms ≈ 20 Hz | CPM-10B 為 slave-only 裝置，同一台不可同時被多個 master 存取 |
| 減少台數（1 台） | ~46 ms ≈ 21 Hz | 改變了量測架構 |

**結論**：現有 3 台 × 38400 baud 配置下，read ≈ 146 ms 是由「韌體延遲 × 請求數 + 傳輸時間」決定的硬常數。提升 baud rate 可壓縮傳輸部分，但 CPM-10B 韌體延遲這塊是電表特性，無法從軟體端消除。
